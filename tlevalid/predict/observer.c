...
#define ELEVATION_ZERO_TOLERANCE 0.01 //threshold for fine-tuning of AOS/LOS
...
predict_julian_date_t predict_next_aos(const predict_observer_t *observer, const predict_orbital_elements_t *orbital_elements, predict_julian_date_t start_time)
{
	...
	if (predict_aos_happens(orbital_elements, observer->latitude) && !predict_is_geostationary(orbital_elements))
	{
